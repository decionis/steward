...
- [docs/ProtocolContracts.md](./docs/ProtocolContracts.md): the plan, decided, that
  ...
  seven workstreams. Decided: the organisation's account comes from the account
  surface at accounts.decionis.com; first boot with the owner's email and the organisation's name
  creates the account and workspace through the Decionis API; the organisation edits its rules
  from Steward and every edit is a new version; two contract items are named for publication.
