...
| Decionis org API key                                        | `DECIONIS_API_KEY` in the server environment, or encrypted in Steward's database                                | High — evaluates decisions, reads dossiers and submits signals and reviews for the organisation; rotate it from the Decionis workspace |
